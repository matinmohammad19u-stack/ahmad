...
from race_system import apply_race, apply_race_to_max_hp, describe_race, random_race
from weapon_system import (
    equip_sword, unequip_sword, total_sword_bonus, get_equipped_list, MAX_EQUIPPED_SWORDS
)
from clothes_shop import CLOTHES_SHOP
from clothes_system import wear_clothing, unwear_clothing, clothing_bonus_for
from pvp_cooldown import pvp_cooldown_remaining, record_pvp_fight, format_cooldown
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        ...
        "/clothes_shop - فروشگاه لباس\n"
        "/ship_shop - فروشگاه کشتی\n"
        "/buy [نام] - خرید\n"
        "/inventory - کیف\n"
        "/equip [نام] - تجهیز شمشیر (تا ۳ تا هم‌زمان)\n"
        "/unequip [نام] - باز کردن یه شمشیر\n"
        "/wear [نام] - پوشیدن لباس\n"
        "/unwear - در آوردن لباس\n"
        "/ship - کشتی فعلی\n"
        "/island - جزیره فعلی\n"
...
# =========================
# =========================
# CHARACTER SELECT - بازنویسی کامل: قبلاً دکمه نشون می‌داد و خودت شخصیت
# انتخاب می‌کردی. الان کاملاً رندومه: سیستم خودش یه شخصیت از بین
# شخصیت‌های هنوز-گرفته‌نشده و یه نژاد (از بین ۱۲۰ نژاد) قرعه‌کشی می‌کنه.
# =========================
async def character_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chosen_race = random_race()
    chosen = characters[chosen_name]
    base_hp = chosen["stats"]["hp"]
    # باف HP نژاد یه‌بار، همینجا، دائمی روی max_hp اعمال می‌شه (نه هر فایت)
    hp = apply_race_to_max_hp(chosen_race, base_hp)
    ...
    cursor.execute(
        "UPDATE players SET character=?, race=?, hp=?, max_hp=? WHERE user_id=?",
        (chosen_name, chosen_race, hp, hp, user.id)
    )
...
async def character(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("""
        SELECT character, hp, max_hp, current_form, awakening,
               equipped_weapons, extra_attack, extra_defense, extra_speed, level, race, equipped_clothing
    ...
    if not data or not data[0]:
        ...
        return
    char_name, hp, max_hp, form, awakening, weapons_str, ex_atk, ex_def, ex_spd, level, race, clothing = data
    ...
    total_atk += total_sword_bonus(weapons_str, char_name)
    total_def += clothing_bonus_for(clothing)
    stats_with_race = apply_race(race, {"attack": total_atk, "defense": total_def, "speed": total_spd})
    total_atk, total_def, total_spd = stats_with_race["attack"], stats_with_race["defense"], stats_with_race["speed"]

    equipped = get_equipped_list(weapons_str)
    weapons_line = ", ".join(equipped) if equipped else "ندارم"
    ...
    await update.message.reply_text(
        ...
        f"{describe_race(race)}\n"
        f"❤️ HP: {hp}/{max_hp}\n"
        f"⚔️ Attack: {total_atk}\n"
        f"🛡️ Defense: {total_def}\n"
        f"💨 Speed: {total_spd}\n"
        f"🗡️ شمشیر(ها) ({len(equipped)}/{MAX_EQUIPPED_SWORDS}): {weapons_line}\n"
        f"👕 لباس: {clothing or 'ندارم'}\n\n"
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("""
        SELECT level, xp, money, points, character, hp, max_hp,
               equipped_weapons, current_ship,
               extra_attack, extra_defense, extra_speed, race, equipped_clothing
        FROM players WHERE user_id=?
    """, (user.id,))
    data = cursor.fetchone()
    if not data or not data[4]:
        await update.message.reply_text("❌ اول /character_select بزن.")
        return
    level, xp, money, points, char_name, hp, max_hp, weapons_str, ship, ex_atk, ex_def, ex_spd, race, clothing = data
    char_stats = characters[char_name]["stats"]
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    online_users[user.id] = user.first_name
    cursor.execute("""
        SELECT level, xp, money, character, hp, max_hp, equipped_weapons,
    ...
    if not data or not data[3]:
        await update.message.reply_text("❌ اول /character_select بزن.")
        return
    level, xp, money, char_name, hp, max_hp, weapons_str, ship, awakening, form = data
    equipped = get_equipped_list(weapons_str)
    weapons_line = ", ".join(equipped) if equipped else "ندارم"
...
async def fight_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, name in enemies.items():
        cd = pvp_cooldown_remaining(user.id, uid)
        if cd > 0:
            cooldown_notes.append(f"⏳ {name} — {format_cooldown(cd)} دیگه مونده")
        else:
            keyboard.append([InlineKeyboardButton(f"⚔️ {name}", callback_data=f"fight_{uid}")])
    if not keyboard:
        text = "❌ الان با هیچ‌کدوم از بازیکنای آنلاین نمی‌تونی فایت بدی (کول‌داون ۱ ساعته):\n"
        text += "\n".join(cooldown_notes)
        await update.message.reply_text(text)
        return
    text = "⚔️ با کی میخوای بجنگی؟"
    if cooldown_notes:
        text += "\n\n" + "\n".join(cooldown_notes)
...
async def fight_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if enemy_id in user_in_battle:
        ...
        return
    # FIX: چک دوباره‌ی کول‌داون همین‌جا هم (در برابر race condition، مثلاً
    # اگه بین باز شدن منوی /fight و زدن دکمه، یکی دیگه زودتر فایت داده باشه)
    cd = pvp_cooldown_remaining(user.id, enemy_id)
    if cd > 0:
        await query.edit_message_text(
            f"❌ با این بازیکن تازه فایت دادی! {format_cooldown(cd)} دیگه مونده تا بشه دوباره."
        )
        return

    cursor.execute(
        "SELECT character, hp, extra_attack, extra_defense, extra_speed, equipped_weapons, current_form, race, equipped_clothing FROM players WHERE user_id=?",
        (user.id,)
    )
    p1 = cursor.fetchone()
    cursor.execute(
    ...
    # مجموع بونوس همه‌ی شمشیرهای تجهیزشده (تا ۳ تا) توی محاسبه‌ی دمیج اعمال می‌شه
    p1_data["stats"]["attack"] += total_sword_bonus(p1[5], p1[0])
    p2_data["stats"]["attack"] += total_sword_bonus(p2[5], p2[0])
    # بونوس Defense لباسِ تجهیزشده
    p1_data["stats"]["defense"] += clothing_bonus_for(p1[8])
    p2_data["stats"]["defense"] += clothing_bonus_for(p2[8])

    p1_form = p1[6] or "Base"
    p2_form = p2[6] or "Base"

    # پسیو + نژاد (باف خودکار و همیشگی، بدون دمیج) اینجا اعمال می‌شن
    p1_data["stats"] = apply_passive(p1[0], p1_data["stats"])
    p2_data["stats"] = apply_passive(p2[0], p2_data["stats"])
    p1_data["stats"] = apply_race(p1[7], p1_data["stats"])
    p2_data["stats"] = apply_race(p2[7], p2_data["stats"])
...
async def fight_attack_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: کول‌داون ۱ ساعته‌ی این جفت پلیر همینجا (وقتی فایت واقعاً تموم
    # شد) ثبت می‌شه؛ تا یه ساعت دیگه این دو نفر نمی‌تونن دوباره با هم
    # فایت بدن (باس‌ها هیچ ربطی به این کول‌داون ندارن).
    record_pvp_fight(winner_id, loser_id)
...
async def boss(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("""
        SELECT character, hp, level, current_form, form_multiplier,
               extra_attack, equipped_weapons, race, extra_defense, equipped_clothing
        FROM players WHERE user_id=?
    """, (user.id,))
    data = cursor.fetchone()
    if not data or not data[0]:
        await update.message.reply_text("❌ اول /character_select بزن.")
        return
    char_name, hp, level, form, form_multiplier, extra_attack, weapons_str, race, extra_defense, clothing = data
    if hp <= 0:
        await update.message.reply_text("❌ HP نداری! از /daily استفاده کن.")
        return
    ...
    # دقیقاً مثل /stats: attack پایه + ارتقا + بونوس شمشیر(ها) + پسیو + نژاد
    total_attack = characters[char_name]["stats"]["attack"] + extra_attack
    total_attack += total_sword_bonus(weapons_str, char_name)
    passive = apply_passive(char_name, {"attack": total_attack})
    total_attack = passive["attack"]
    total_attack = apply_race(race, {"attack": total_attack})["attack"]

    # بونوس Defense شخصیت (ارتقا + لباس) — برای محاسبه‌ی دمیجی که از boss می‌خوری
    total_defense = characters[char_name]["stats"]["defense"] + extra_defense
    total_defense += clothing_bonus_for(clothing)
    ...
    while player_hp > 0 and boss_hp > 0 and turn <= 100:
        ...
        # FIX: قبلاً Defense شخصیت (از /upgrade_defense یا لباس) هیچ تأثیری
        # روی دمیجی که از boss می‌خوردی نداشت. الان دقیقاً با همون فرمول
        # بقیه‌ی جاهای پروژه (compute_damage.py: enemy_defense * 0.3) کم می‌شه.
        b_dmg = max(1, int(boss_attack * random.uniform(0.8, 1.3)) - int(total_defense * 0.3))
        player_hp -= b_dmg
...
# =========================
# CLOTHES SHOP - دقیقاً همون الگوی صفحه‌بندیِ فروشگاه شمشیر (که خودش یه
# FIX بود برای جلوگیری از رد شدن سقف ۴۰۹۶ کاراکتریِ پیام تلگرام)، اینجا
# هم برای فروشگاه لباس تکرار شده.
# =========================
CLOTHES_PER_PAGE = 8


def _clothes_shop_total_pages() -> int:
    return max(1, (len(CLOTHES_SHOP) + CLOTHES_PER_PAGE - 1) // CLOTHES_PER_PAGE)


def _clothes_shop_page_text(page: int) -> str:
    names = list(CLOTHES_SHOP.keys())
    total_pages = _clothes_shop_total_pages()
    page = max(0, min(page, total_pages - 1))
    start = page * CLOTHES_PER_PAGE
    chunk = names[start:start + CLOTHES_PER_PAGE]

    text = f"👕 فروشگاه لباس (صفحه {page + 1}/{total_pages} | مجموع {len(CLOTHES_SHOP)} لباس)\n\n"
    for name in chunk:
        item = CLOTHES_SHOP[name]
        text += f"👕 {name}\n   DEF: +{item['defense']} | {item['price']} 💰\n   {item.get('description', '')}\n\n"
    text += "برای خرید: /buy [نام لباس]\nبرای پوشیدن: /wear [نام لباس]"
    return text


def _clothes_shop_keyboard(page: int) -> InlineKeyboardMarkup:
    total_pages = _clothes_shop_total_pages()
    page = max(0, min(page, total_pages - 1))
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton("⬅️ قبلی", callback_data=f"clopg_{page - 1}"))
    nav_row.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="clopg_noop"))
    if page < total_pages - 1:
        nav_row.append(InlineKeyboardButton("بعدی ➡️", callback_data=f"clopg_{page + 1}"))
    return InlineKeyboardMarkup([nav_row])


async def clothes_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    online_users[user.id] = user.first_name
    await update.message.reply_text(
        _clothes_shop_page_text(0),
        reply_markup=_clothes_shop_keyboard(0)
    )


async def clothes_shop_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.data == "clopg_noop":
        return
    try:
        page = int(query.data.replace("clopg_", ""))
    except ValueError:
        return
    await query.edit_message_text(
        _clothes_shop_page_text(page),
        reply_markup=_clothes_shop_keyboard(page)
    )
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if item_name in SHOP_ITEMS:
    ...
    elif item_name in CLOTHES_SHOP:
        clothing = CLOTHES_SHOP[item_name]
        money = get_money(user.id)
        if money < clothing["price"]:
            ...
            cursor.execute(
                "UPDATE players SET money=money-? WHERE user_id=?",
                (clothing["price"], user.id)
            )
            add_item(user.id, item_name, "clothing")
            db.commit()
            result = f"✅ {item_name} خریداری شد! 👕 DEF+{clothing['defense']}\nبرای پوشیدنش: /wear {item_name}"
...
# =========================
# EQUIP - بازنویسی کامل: قبلاً فقط یه شمشیر هم‌زمان می‌شد تجهیز کرد
# (equipped_weapon تک‌ستونی، هر بار جایگزین قبلی می‌شد). الان تا ۳ تا
# شمشیر هم‌زمان قابل‌تجهیزه (equipped_weapons)، دقیقاً مثل سبک سه‌شمشیرِ
# زوروِ واقعی. برای باز کردن یکی از ۳ تا، /unequip [نام شمشیر].
# =========================
async def equip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    online_users[user.id] = user.first_name
    if not context.args:
        await update.message.reply_text(f"❌ بنویس: /equip [نام شمشیر] (تا {MAX_EQUIPPED_SWORDS} تا هم‌زمان)")
        return
    item_name = " ".join(context.args)
    result = equip_sword(user.id, item_name)
    await update.message.reply_text(result)
...
async def unequip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    online_users[user.id] = user.first_name
    if not context.args:
        await update.message.reply_text("❌ بنویس: /unequip [نام شمشیر]")
        return
    item_name = " ".join(context.args)
    result = unequip_sword(user.id, item_name)
    await update.message.reply_text(result)

# =========================
# CLOTHES - لباس، برخلاف شمشیر، فقط یه دونه هم‌زمان تنته و DEF می‌ده
# (نه ATK). با /wear می‌پوشیش، با /unwear درش میاری.
# =========================
async def wear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    online_users[user.id] = user.first_name
    if not context.args:
        await update.message.reply_text("❌ بنویس: /wear [نام لباس]")
        return
    item_name = " ".join(context.args)
    result = wear_clothing(user.id, item_name)
    await update.message.reply_text(result)


async def unwear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    online_users[user.id] = user.first_name
    result = unwear_clothing(user.id)
    await update.message.reply_text(result)
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/shop - فروشگاه\n"
        "/sword_shop - فروشگاه شمشیر\n"
        "/clothes_shop - فروشگاه لباس (بونوس Defense)\n"
        "/ship_shop - فروشگاه کشتی\n"
        "/buy [نام] - خرید\n"
        "/inventory - کیف\n"
        "/equip [نام] - تجهیز شمشیر (تا ۳ تا هم‌زمان، مثل سه‌شمشیرِ زورو)\n"
        "/unequip [نام] - باز کردن یکی از شمشیرهای تجهیزشده\n"
        "/wear [نام] - پوشیدن لباس (فقط یکی هم‌زمان)\n"
        "/unwear - در آوردن لباس\n"
        "/ship - کشتی فعلی\n"
        "/island - جزیره فعلی\n"
...
app.add_handler(CommandHandler("clothes_shop", clothes_shop))
...
app.add_handler(CommandHandler("wear", wear))
app.add_handler(CommandHandler("unwear", unwear))
...
app.add_handler(CallbackQueryHandler(clothes_shop_page_callback, pattern=r"^clopg_"))
