...
# SAVE FIGHT RESULT
# =========================
def save_fight(user_id, enemy, result, levels_gained, money):
    cursor.execute("""
        INSERT INTO fight_history (user_id, enemy, result, reward_xp, reward_money)
        VALUES (?, ?, ?, ?, ?)
    """, (user_id, enemy, result, levels_gained, money))
    db.commit()
...
# GIVE LEVELS
# =========================
def give_levels(user_id, levels):
    cursor.execute("""
        UPDATE players SET level = level + ?, xp = 0
        WHERE user_id = ?
    """, (levels, user_id))
    db.commit()
...
def fight(user_id, enemy_name, player_skill_damage):
    ...
    while player_hp > 0 and enemy_hp > 0:
        ...
        enemy_hp -= dmg
        log.append(f"🟦 Turn {turn}: You hit {dmg} dmg" + (" 💥 CRIT!" if crit == 2 else ""))
    ...
    if player_hp > 0:
        ...
        levels_gained = 5
        money = 100 + level * 20
        msg = f"🏆 YOU WIN! +{levels_gained} لول 💰 +{money}"
    else:
        result = "LOSE"
        levels_gained = 0
        money = 20
    ...
    save_fight(user_id, enemy_name, result, levels_gained, money)
    update_hp(user_id, max(0, player_hp))

    if result == "WIN":
        give_levels(user_id, levels_gained)
        cursor.execute("UPDATE players SET money=money+? WHERE user_id=?", (money, user_id))
        db.commit()
